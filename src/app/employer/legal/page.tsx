...
export default function EmployerLegalHubPage() {
  ...
  const legalItems = [
    {
      ...
      subtitle: 'How we collect, encrypt, and protect your personal data & candidate proofs.',
      icon: Lock,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 pb-20 font-sans animate-fade-in">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              <span>{activeTab === 'directory' ? 'Employer Legal & Privacy Hub' : legalItems.find(i => i.id === activeTab)?.title || 'Platform Policy'}</span>
            </h1>
            <p className="text-[11px] text-slate-500 font-semibold">
              Official platform policies, employer subscription rules &amp; compliance.
            </p>
          </div>
        </div>
        ...
        <>
          <div className="bg-white rounded-3xl border border-slate-100 shadow-xs overflow-hidden divide-y divide-slate-100">
            <div className="px-4 py-3 bg-slate-50/70 border-b border-slate-100 flex items-center justify-between">
              <span className="text-[11px] font-black text-slate-800 uppercase tracking-wider">Employer Policies Directory</span>
            ...
            {legalItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left p-4 flex items-center justify-between hover:bg-blue-50/40 transition-colors group cursor-pointer"
                >
                  <div className="flex items-center gap-3.5 min-w-0 flex-1">
                    <div className="p-2.5 rounded-2xl bg-blue-50 text-[#1A73E8] group-hover:bg-[#1A73E8] group-hover:text-white transition-colors shrink-0">
                  ...
                  <ChevronRight size={16} className="text-slate-400 group-hover:translate-x-1 group-hover:text-[#1A73E8] transition-all shrink-0" />
                </button>
              );
            })}
          </div>
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[10.5px] text-amber-900 font-medium leading-relaxed">
                  Request permanent erasure of your employer profile and hiring data at any time.
                </p>
              </div>
            </div>
      ...
      {activeTab === 'pricing' && <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs"><PricingContent /></div>}
      {activeTab === 'refunds' && <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs"><RefundsContent /></div>}
      {activeTab === 'shipping' && <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs"><ShippingContent /></div>}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-3xl max-w-sm w-full p-5 space-y-3 shadow-2xl border border-slate-100">
            ...
            <div className="space-y-2 text-xs text-slate-600 font-medium leading-relaxed">
              <p>To request permanent employer account and data deletion:</p>
              <ol className="list-decimal list-inside space-y-1 bg-slate-50 p-2.5 rounded-2xl border border-slate-100 font-semibold text-slate-800 text-[10.5px]">
                <li>Option 1: Employer Account → Danger Zone → Request Account Deletion.</li>
