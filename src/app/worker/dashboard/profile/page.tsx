...
export default function WorkerProfilePage() {
  const { 
    ...
    handleSaveProfile, handleRequestAccountDeletion 
  } = useWorkerDashboard();

  const [name, setName] = useState(workerProfile.name || 'Janhvi');
  const [expectedSalary, setExpectedSalary] = useState(workerProfile.expectedSalary || '18000');
  const [experience, setExperience] = useState(workerProfile.experience || '4 Years');
  const [phone, setPhone] = useState(workerProfile.phone?.replace(/\D/g, '').slice(-10) || '9876543210');
  const [languages, setLanguages] = useState<string[]>(workerProfile.languages || ['Hindi', 'English']);
  ...
  const handlePhoneChange = (val: string) => {
    // Only digits allowed, exactly max 10 digits
    ...
    <div className="space-y-6 animate-fade-in max-w-3xl pb-12">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 font-semibold mt-0.5">
          Manage your candidate profile information and household preferences.
        </p>
      </div>

      {/* Profile Edit Card */}
      <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider">Candidate Details</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-bold">
          ...
          <div className="space-y-1">
            <label className="text-slate-500 text-[10px] uppercase">Experience Years</label>
            <input 
              type="text" 
              value={experience} 
