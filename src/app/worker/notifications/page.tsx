    ...
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto pb-20">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-2xl bg-blue-50 text-[#1A73E8] flex items-center justify-center font-bold relative">
              <Bell size={18} />
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#EA4335] text-white text-[9px] font-semibold flex items-center justify-center border-2 border-white">
                  {unreadCount}
                </span>
              )}
            </div>
            <span>{t('notificationsCenterTitle')}</span>
          </h2>
          <p className="text-xs text-slate-500 font-medium mt-1">
            {t('notificationsCenterSub')}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (
            <button
              ...
              className="py-2 px-3.5 bg-blue-50 hover:bg-blue-100 text-[#1A73E8] rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95"
            >
              <Check size={14} strokeWidth={2.5} />
              <span>{t('markAllReadBtn')}</span>
            </button>
          )}
          ...
          {notifications.length > 0 && (
            <button
              ...
              className="py-2 px-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-medium transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
            >
              <Trash2 size={13} />
              <span>{t('clearNotifsBtn')}</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-slate-100 pb-2">
        <button
          ...
          className={`py-2 px-4 rounded-2xl text-xs font-semibold transition-all cursor-pointer ${
            activeTab === 'all'
              ? 'bg-slate-900 text-white shadow-sm'
              : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
          }`}
        >
          {t('allFilter')} ({notifications.length})
        </button>
        ...
        >
          {t('navInterviews')} ({notifications.filter(n => n.type === 'interview_scheduled').length})
        </button>
      </div>
      ...
      <div className="space-y-3">
        {filteredNotifications.length === 0 ? (
          <div className="bg-white p-8 rounded-3xl border border-slate-100 text-center space-y-2 shadow-xs">
            ...
            <h3 className="text-sm font-semibold text-slate-800">{t('noNotifsFoundTitle')}</h3>
            <p className="text-xs text-slate-400 font-medium">{t('noNotifsFoundSub')}</p>
          </div>
        ) : (
                ...
                {n.actionUrl && (
                  <div className="flex justify-end pt-2 border-t border-slate-100/60">
                    <Link
                      ...
                      className={`py-2 px-4 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95 ${
                        isInterview 
