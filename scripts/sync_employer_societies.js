...
console.log("🔗 Connecting to PostgreSQL host:", connectionString ? connectionString.split('@')[1] : 'MISSING DB URL');
...
async function runSync() {
  ...
  try {
    ...
    const epSyncRes = await client.query(`
    ...
    console.log(`\n✅ Synced ${epSyncRes.rows.length} employer profiles in public.employer_profiles:`);
    if (epSyncRes.rows.length > 0) {
      console.table(epSyncRes.rows);
    }

    // 3. Sync society_name from public.jobs / public.societies to public.profiles
    const pSyncRes = await client.query(`
      ...
      WHERE p.id::text = j.employer_id::text
        ...
        AND (j.society_name IS NOT NULL OR s.name IS NOT NULL)
      RETURNING p.id, p.society_name;
    `);

    console.log(`\n✅ Synced ${pSyncRes.rows.length} user profiles in public.profiles:`);
    if (pSyncRes.rows.length > 0) {
    ...
    console.log("\n📊 Current Employer Societies Summary in PostgreSQL:");
    console.table(summary.rows);

    console.log("\n🎉 Employer Society Sync Executed Successfully!");
  } catch (err) {
